    ...
    <section className="min-h-screen bg-background text-foreground overflow-hidden">
      ...
      >
        {featuredProjects.map((proj, idx) => {
          const p = positions[idx] || { left: "0%", top: "0%", width: "auto", rotate: 0, smRotate: 0 };
          ...
          return (
            ...
            >
              ...
              <span className="text-[9px] border px-2 py-1 select-none">FEATURED</span>
              ...
              <div className="flex justify-between mt-8 pt-4 border-t border-dashed select-none">
                ...
                <div className="flex gap-2 pointer-events-auto relative z-10">
                  {proj.github && (
                    <a href={proj.github} target="_blank" rel="noreferrer" className="p-2 border bg-card hover:bg-muted">
                      <FaGithub size={12} />
                    </a>
                  )}

                  {proj.link && (
                    <a href={proj.link} target="_blank" rel="noreferrer" className="p-2 border bg-card hover:bg-muted">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          style={{ left: "42%", top: "35%", transform: "rotate(-2deg)" }}
        ...
        >
          <object
            data="/svgwaves_io_one-piece.svg"
            type="image/svg+xml"
      ...
      <div className="max-w-6xl mx-auto px-4 ">
        <div className="flex justify-between border-b pb-4 mb-6">
          ...
          <span className="text-xs opacity-50 font-mono">{archiveProjects.length}</span>
        </div>
        ...
        {archiveProjects.map((proj) => (
          <div key={proj.id} className="flex justify-between py-4 border-b">
            <div>
              <h3>{proj.name}</h3>
              <span className="text-xs opacity-50">{proj.tags.join(" · ")}</span>
            </div>
            <div className="flex gap-4">
              {proj.github && <a href={proj.github} target="_blank" rel="noreferrer"><FaGithub /></a>}
              {proj.link && <a href={proj.link} target="_blank" rel="noreferrer"><FaExternalLinkAlt /></a>}
            </div>
          </div>
        ))}
      </div>
